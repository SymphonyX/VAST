...
//For deriving different planners and selecting the one we are using
 public class Planner : MonoBehaviour{
	...
	[HideInInspector] public FootstepPlanningState currentState;
	...
	{ 
		FootstepPlanningAction[] a = new FootstepPlanningAction[1] {default(FootstepPlanningAction)}; 
		return a;
	}
	...
	public virtual bool CheckDomainStateCollisions(int d, FootstepPlanningState st)
	{return false;}
	
	public virtual bool ComputePlan(){return false;}
	
	public virtual FootstepPlanningAction getFirstAction()
	{return default(FootstepPlanningAction); }
};
 ...
 interface IPlannerInterface<StateMap>
{
	...
	public virtual Vector3 statePosition() { return new Vector3(); }
...
public class BestFirstSearchNode {
	...
	public BestFirstSearchNode(float _g, float _f, ref DefaultState _previousStateRef, ref DefaultAction _nextActionRef) 
...
public class PlanningDomainBase {
		...
		public virtual void clearAtBeginningOfEveryPlanIteration ()
		{
...
class BestFirstSearchPlanner : IPlannerInterface<Dictionary<DefaultState, BestFirstSearchNode>> {
	...
	//I cannot use SortedHash for some reason, so I need to remember sorting the hash by "f" whenever it's updated
...
//		int index = 0;
        ...
        {			
            ...
            foreach (DefaultAction action in possibleActions)
            {
                ...
                nextNode = new BestFirstSearchNode(newg, newf, ref x.action.state, ref nextAction);				
...
//		int index = 0;
	...
	public bool computePlan(ref DefaultState startState, ref DefaultState goalState, ref Stack<DefaultState> plan, float maxTime )
    {
		...
		if(oneStep){
			if(initPlanner){
				  float newf = _planningDomain[0].estimateTotalCost(ref startState, ref goalState, 0.0f);
				  BestFirstSearchNode nextNode = new BestFirstSearchNode(0.0f, newf, ref startState, ref startState);
			      Visited[nextNode.action.state] = nextNode;
		          openSet.Add(nextNode);
				  initPlanner = false;
			}
			isPlanComplete = _computeOneStep(ref startState, ref goalState, Visited, ref stateReached, maxTime);
			s = stateReached;
		}
		else {
			isPlanComplete = _computePlan(ref startState, ref goalState, Visited, ref s, maxTime);
		}
        ...
        do {
            ...
            foreach(KeyValuePair<DefaultState, BestFirstSearchNode> node in Visited)
            {
	...
	public void VisualizeContainer(ContainerType containerType, Color color, float radius)
	{
		...
		switch(containerType){
			case ContainerType.Close:
				Debug.LogWarning("BestFirstPlanner does not implement close list");
				break;
	...
	void showStateMap(float radius, Color color)
	{
		if(Visited.Count == 0)
			Debug.LogWarning("Visited List is Empty");
		else {
			foreach(BestFirstSearchNode node in Visited.Values)
			{
				Vector3 nodePosition = node.action.state.statePosition();
				Vector3 direction = node.action.state.statePosition() - node.previousState.statePosition();
				Gizmos.color = color;
	...
	void showPlan(float radius, Color color)
	{
		if(outputPlan.Count == 0)
			...
			foreach(BestFirstSearchNode node in outputPlan)
			{
				Vector3 nodePosition = node.action.state.statePosition();
				Vector3 direction = node.action.state.statePosition() - node.previousState.statePosition();
				Gizmos.color = color;
